...
- **v1.12.0** (2026-08-12) — **The release workflow's `paths: ['VERSION']` filter is gone, and a gate now asserts that a release was actually cut.** v1.11.0 recorded this repo as carrying instance 5 of `green-gate-never-examined-the-case` and left the fix out; this is the fix. **The cost here was and remains nil, re-verified rather than inherited:** `309d720` (2026-05-29) introduced `VERSION` and `release.yml` in the *same* commit at `VERSION=1.7.0`, and all nine values VERSION has held since have a release — git tags likewise start at `v1.7.0`, so there are no orphan tags either. **Why remove a filter that has cost nothing:** (1) it converts any *transient* miss into a permanent one, which is the general form of the upstream v1.13.0 bug rather than a special case — filtered, the job's only retry is the next change to `VERSION`, by which time `VERSION` holds a different value and the skipped one can never be cut; unfiltered, every later push to `main` re-attempts it, so a run lost to a token blip or a runner outage self-heals. (2) **The guard that caught it upstream does not exist here.** `validate_plugins.py`'s VERSION drift guard only compares `VERSION` against a plugin version in a *single-plugin* repo (its rule 7); this bundle ships ten plugins, so a commit bumping the manifests and this changelog while leaving `VERSION` behind produces **no red guard at all** — not even the belated signal that fired upstream on `43f4d62`. **New gate:** `.github/scripts/check_release_parity.py`, vendored byte-identical from [`agent-traffic-control`](https://github.com/wan-huiyan/agent-traffic-control) `0b9ac0a` (upstream v1.23.1, sha256 `9afdae47…`) behind the usual `--8<--` markers, wired into `ci.yml` and **`fetch-depth: 0` added to the checkout it depends on** — without it `git log --follow VERSION` sees one commit and the VERSION half of the gate's union contributes nothing while staying green (measured upstream at 26 claimed versions on the runner against 28 locally). It unions VERSION history with this changelog because **neither source alone** sees the failure it was written for. Baselined in `.release-parity-accepted`: seven accepted holes, `v1.0.0`–`v1.6.0`, every one dated 2026-04-16 to 2026-04-25 and therefore before `309d720`. Deliberately **not** copied from upstream's baseline is its "cause unestablished" wording — there, `release.yml` demonstrably existed for three of its holes and the misses are unexplained; here the workflow's own introducing commit set `VERSION=1.7.0` and `v1.7.0` has a release, so no such window exists and a borrowed reason would have been a confident wrong note. **The gate was made to fail on purpose before it shipped** — injecting a changelog bullet for an unreleased `v1.99.0` took it 0 → 1 with the version named, and removing it took it back to 0; the transcript is in the PR. (Written without the bold markers on purpose — the gate's entry regex is line-anchored so a mid-sentence mention cannot match it, but a bullet-shaped one could if this paragraph were ever reflowed.) **Known limit, recorded in the vendoring note:** the changelog half needs an exactly-`## Version history` heading *and* `- **vX.Y.Z**` bullets, so a table, an emoji heading or a per-component changelog silently parses to zero entries. Measured across eleven repos, five read zero — which is why this file lands here, where the README satisfies both conditions and 16 entries are read, and **not** in the sibling repos that share the workflow bug. Tracked as [`agent-traffic-control#41`](https://github.com/wan-huiyan/agent-traffic-control/issues/41). No skill description changed; the listing total is unchanged.
